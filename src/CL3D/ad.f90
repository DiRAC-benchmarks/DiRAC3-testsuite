...
!$OMP END PARALLEL DO
  ...
  tmpFlopCount=tmpFlopCount+((z_max-z_min+1+4)*(y_max-y_min+1+4)*(x_max-x_min+1+4)*6) !sc - 16464
...
!$OMP PARALLEL DO
    ...
    ENDDO
!$OMP END PARALLEL DO
  tmpFlopCount=tmpFlopCount+((z_max-z_min+1+4)*(y_max-y_min+1+4)*(x_max-x_min+1+4)*6)
...
!$OMP PARALLEL DO
    ...
    ENDDO
!$OMP END PARALLEL DO
 tmpFlopCount=tmpFlopCount+((z_max-z_min+1+4)*(y_max-y_min+1+4)*(x_max-x_min+1+4)*4)
...
!$OMP PARALLEL DO
    ...
    ENDDO
!$OMP END PARALLEL DO
  tmpFlopCount=tmpFlopCount+((z_max-z_min+1+4)*(y_max-y_min+1+4)*(x_max-x_min+1+4)*2)
...
!$OMP PARALLEL DO
      ...
      ENDDO
!$OMP END PARALLEL DO
  tmpFlopCount=tmpFlopCount+((z_max-z_min+2)*(y_max-y_min+2)*(x_max-x_min+1+4)*8)
!And do I need to calc the node mass for all 3 directions, or just once?
   flopCount=flopCount+tmpFlopCount !sc 27664
   tmpFlopCount=0 
...
!$OMP END PARALLEL DO
      ...
      tmpFlopCount=tmpFlopCount+((z_max-z_min+2)*(y_max-y_min+2)*(x_max-x_min+1+3)*16)
      flopCount=flopCount+tmpFlopCount !sc 48464
      tmpFlopCount=0
...
!$OMP PARALLEL DO
      ...
      ENDDO
!$OMP END PARALLEL DO
     tmpFlopCount=tmpFlopCount+((z_max-z_min+2)*(y_max-y_min+2)*(x_max-x_min+4)*2)
     flopCount=flopCount+tmpFlopCount !sc 51064
     tmpFlopCount=0
...
!$OMP PARALLEL DO PRIVATE(upwind,downwind,donor,dif,sigma,width,limiter,vdiffuw,vdiffdw,auw,adw,wind) REDUCTION (+:tmpFlopCount)
    DO l=z_min,z_max+1
      DO k=y_min,y_max+1
...
!$OMP END PARALLEL DO
   ...
   tmpFlopCount=tmpFlopCount+((z_max-z_max+2)*(y_max-y_min+2)*(x_max-x_min+3)*10)
   flopCount=flopCount+tmpFlopCount
   tmpFlopCount=0
...
!$OMP END PARALLEL DO
  ...
  tmpFlopCount=tmpFlopCount+((z_max-z_min+2)*(y_max-y_min+2)*(x_max-x_min+2)*7)
  flopCount=flopCount+tmpFlopCount
  tmpFlopCount=0
...
!$OMP END PARALLEL DO
      ...
      tmpFlopCount=tmpFlopCount+((z_max-z_min+2)*(y_max-y_min+5)*(x_max-x_min+2)*8)
      flopCount=flopCount+tmpFlopCount
      tmpFlopCount=0
...
!$OMP PARALLEL DO
      ...
      ENDDO
!$OMP END PARALLEL DO
      tmpFlopCount=tmpFlopCount+((z_max-z_min+2)*(y_max-y_min+4)*(x_max-x_min+2)*16)
      flopCount=flopCount+tmpFlopCount
      tmpFlopCount=0 
!$OMP PARALLEL DO
      DO l=z_min,z_max+1
...
!$OMP END PARALLEL DO
    ...
    tmpFlopCount=tmpFlopCount+((z_max-z_min+2)*(y_max-y_min+4)*(x_max-x_min+2)*2)
    flopCount=flopCount+tmpFlopCount
    tmpFlopCount=0
    ENDIF
!$OMP PARALLEL DO PRIVATE(upwind,donor,downwind,dif,sigma,width,limiter,vdiffuw,vdiffdw,auw,adw,wind) REDUCTION (+:tmpFlopCount)
    DO l=z_min,z_max+1
...
!$OMP END PARALLEL DO
      ...
      tmpFlopCount=tmpFlopCount+((z_max-z_min+5)*(y_max-y_min+2)*(x_max-x_min+2)*8)
      flopCount=flopCount+tmpFlopCount
      tmpFlopCount=0
...
!$OMP PARALLEL DO
      ...
      ENDDO
!$OMP END PARALLEL DO
      tmpFlopCount=tmpFlopCount+((z_max-z_min+4)*(y_max-y_min+2)*(x_max-x_min+2)*16)
      flopCount=flopCount+tmpFlopCount
      tmpFlopCount=0
...
DO l=z_min-1,z_max+2
      ...
      ENDDO
!$OMP END PARALLEL DO
    tmpFlopCount=tmpFlopCount+((z_max-z_min+4)*(y_max-y_min+2)*(x_max-x_min+2)*2)
