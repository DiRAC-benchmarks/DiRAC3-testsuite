...
SUBROUTINE advec_mom_kernel(mom_flop,                           &
  ...
  INTEGER :: x_min,x_max,y_min,y_max,z_min,z_max,gz,gx,gy 
...
iter=(gz-z_min+5)*(gy-y_min+5)*(gx-x_min+5)
...
!$OMP PARALLEL DO
      ...
      ENDDO
!$OMP END PARALLEL DO
  iter=(gz-z_min+2)*(gy-y_min+2)*(gx-x_min+5)
  fc=iter*8
...
!$OMP END PARALLEL DO
      ...
      iter=(gz-z_min+2)*(gy-y_min+2)*(gx-x_min+4)
      fc=iter*16
      flop=flop+fc
...
!$OMP PARALLEL DO PRIVATE(upwind,downwind,donor,dif,sigma,width,limiter,vdiffuw,vdiffdw,auw,adw,wind) REDUCTION (+:limiter_count)
    ...
    ENDDO
!$OMP END PARALLEL DO
   iter=(gz-z_min+2)*(gy-y_min+2)*(gx-x_min+3)
   fc=(iter*10)+(limiter_count*19)
   flop=flop+fc
...
!$OMP PARALLEL DO
    ...
    ENDDO
!$OMP END PARALLEL DO
  iter=(gz-z_min+2)*(gy-y_min+2)*(gx-x_min+2)
  fc=iter*7
  flop=flop+fc
...
!$OMP END PARALLEL DO
      ...
      iter=(gz-z_min+2)*(gy-y_min+5)*(gx-x_min+2)
      fc=iter*8
      flop=flop+fc
...
!$OMP END PARALLEL DO
      ...
      iter=(gz-z_min+2)*(gy-y_min+4)*(gx-x_min+2)
      fc=iter*16
      flop=flop+fc
...
!$OMP PARALLEL DO PRIVATE(upwind,donor,downwind,dif,sigma,width,limiter,vdiffuw,vdiffdw,auw,adw,wind) REDUCTION (+:limiter_count)
    ...
    ENDDO
!$OMP END PARALLEL DO
  iter=(gz-z_min+2)*(gy-y_min+3)*(gx-x_min+2)
  fc=(iter*9)+(limiter_count*19)
  flop=flop+fc
...
!$OMP END PARALLEL DO
      ...
      iter=(gz-z_min+5)*(gy-y_min+2)*(gx-x_min+2)
      fc=iter*8
      flop=flop+fc
...
!$OMP PARALLEL DO
      ...
      ENDDO
!$OMP END PARALLEL DO
      iter=(gz-z_min+4)*(gy-y_min+2)*(gx-x_min+2)
      fc=iter*16
      flop=flop+fc
