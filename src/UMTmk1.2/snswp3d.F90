...
!***********************************************************************
   ...
   subroutine snswp3d(Groups, Angle,         &
                      next, nextZ, psic, psib, omega, chk)
...
!  Arguments
   ...
   real(adqt), intent (out)  :: chk

!  Local Variables

     type(ZoneData), pointer :: Z  ! Z => Geom %ZData(zone)
...
!  Constants
   ...
   nzones    = Size%nzones
write (6,*) "Enter snswp3d "
write (6,*) "nzones = ", nzones
...
!  Loop through all of the corners using the NEXT list
   ...
   ZoneLoop: do ii=1,nzones
     ...
     zone = nextZ(ii)
!    write (6,*) ' ii = ',ii, ' nextZ(ii) = ', zone

!    Z    => getZoneData(Geom, zone)
     Z => Geom %ZData(ii  )
!    write (6,*) ' Did Z => Geom %ZData(zone) '
!    write (6,*) ' Z % Sigt(Size% ngr ', Z % Sigt(Size% ngr)
!    write (6,* ) ' Z % SigtInv(Size% ngr) ', Z % SigtInv(Size% ngr) 
!    write (6,*) ' Z % A_fp(Size% ndim,Z% nCFaces,Z% nCorner) ', Z % A_fp(Size% ndim,Z% nCFaces,Z% nCorner) 
!    write (6,*) ' Z % A_ez(Size% ndim,Z% nCFaces,Z% nCorner) ', Z % A_ez(Size% ndim,Z% nCFaces,Z% nCorner) 


     nCorner   = Z% nCorner
     nCFaces   = Z% nCFaces
     c0        = Z% c0 
!    write (6,*) ' nCorner  nCFaces c0 ', nCorner, nCFaces, c0
...
!  corner faces (FP faces)
       ...
       do icface=1,ncfaces
!        write (6,*) ' Doing icface ', icface
!        write (6,*) 'Z%A_fp(1,icface,c) ', Z%A_fp(1,icface,c)
...
!write (6,*) ' After   Calculate Area_CornerFace dot Omega to determine the  contributions from incident fluxes across external  corner faces (FP faces) '

!  Contributions from interior corner faces (EZ faces)
!write (6,*) ' !  Contributions from interior corner faces (EZ faces) '
